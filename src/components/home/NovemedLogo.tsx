...
export function NovemedLogo({ variant = 'header', theme = 'light', className = '' }: Props) {
  ...
  return (
    <div className={`select-none ${scale} ${className}`}>
      ...
      <p
        className={`mt-1 text-[11px] font-semibold leading-snug tracking-[0.14em] sm:mt-0.5 sm:text-xs sm:tracking-[0.2em] ${tagline}`}
      >
        INNOVACIÓN MÉDICA
      </p>
      <p className="mt-0.5 text-[10px] font-medium text-novemed-blue sm:text-[10px]">J-301763445</p>
    </div>
  )
}
